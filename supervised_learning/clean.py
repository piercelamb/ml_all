...
def final_plots(run_type, clf, clf_type, cv, score, X_train, y_train, X_test, y_test, y_pred, smote):
    ...
    plot_learning_curve(clf, run_type+' '+smote_text+' Learning Curve (' + clf_type + ')', X_train, y_train, cv=cv, scoring=score,
                        filename=run_type+'_'+smote_text+'_'+clf_type + '_' + score + '_Learning_Curve.png')

def initialize_a_param(clf, param, X_train, y_train, cv, clf_type, scoring):
...
def get_classifier(clf_type, param_name, option, param2, smote):
    if param2 != None:
        ...
        kwargs = {param_name: option, 'random_state': RANDOM_STATE, param2_name: param2_val, 'n_jobs':-1}
    else:
        kwargs = {param_name: option, 'random_state': RANDOM_STATE, 'n_jobs':-1}
...
def validate_two_params(param1, param2, clf, run_type, clf_type, scoring, cv,X_train, y_train, X_test, y_test, smote):
    ...
    param_2_initial_val = initialize_a_param(clf, param2, X_train, y_train, cv, clf_type, scoring)
    ...
    kwargs = {param1_name: best_param1_value, param2_name: best_param2_value, 'random_state': RANDOM_STATE, 'n_jobs':-1}
    return kwargs
...
def validate_single_param(param1, param2, run_type, clf_type, scoring, cv,X_train, y_train, X_test, y_test, smote):
    best_param_value = get_validation_curve(run_type, scoring, cv, clf_type, X_train, y_train, X_test, y_test, param1,
                                            param2, smote)
    param_name = list(param1)[0]
    kwargs = {param_name: best_param_value, 'random_state': RANDOM_STATE, 'n_jobs':-1}
    return kwargs
...
def run_dt(X_train, y_train, X_test, y_test, scoring, cv, smote, run_type, param1, param2):
    ...
    final_plots(run_type, clf, clf_type, cv, scoring, X_train, y_train, X_test, y_test, y_pred, smote)
...
def run_shoppers(dataroot):
    ...
    #     {'hidden_layer_sizes': [(100,), (150,), (200,), (100,100,100)]},  # validation_curve
